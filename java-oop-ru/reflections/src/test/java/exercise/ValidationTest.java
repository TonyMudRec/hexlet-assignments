...
class ValidationTest {
    ...
    void testAdvancedValidate() {
        ...
        assertThat(result1).as("first test").isEqualTo(expected1);

        Address address2 = new Address("United States", "Texas", null, "7", "2");
        Map<String, List<String>> result2 = Validator.advancedValidate(address2);
        Map<String, List<String>> expected2 = Map.of("street", List.of("can not be null"));
        assertThat(result2).as("second test").isEqualTo(expected2);

        Address address3 = new Address("USA", "Texas", "Musicians Street", "7", "2");
        ...
        assertThat(result3).as("third test").isEqualTo(expected3);

        Address address4 = new Address("Russia", null, "St", "1", null);
        Map<String, List<String>> result4 = Validator.advancedValidate(address4);
        Map<String, List<String>> expected4 = Map.of("city", List.of("can not be null")
                , "street", List.of("length less than 3"));
        assertThat(result4).as("fourth test").isEqualTo(expected4);
    }
    // END
}
