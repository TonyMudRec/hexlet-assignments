...
public class CommentsController {
    ...
    public Comment show(@PathVariable String id) {
        ...
        return maybeComment.orElseThrow(() -> new ResourceNotFoundException("Comment with id " + id + " not found"));
    }
...
//    @ResponseStatus(HttpStatus.)
    public Comment update(@PathVariable String id, @RequestBody Comment data) {
        var maybePost = commentRepository.findById(Long.parseLong(id));
        if (maybePost.isPresent()) {
            Comment comment = maybePost.get();
